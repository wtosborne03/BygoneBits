				...
				if (Physics.Raycast(m_transform.position, m_transform.forward, out hitInfo, m_grabMaxDistance, m_collisionMask))
				{
					...
					if ((mdown && !omdown))
					{
						...
						case 12:
							...
							if ((mdown && !omdown))
							{
								Instantiate(pickeffect, hitInfo.collider.transform.position, Quaternion.Euler(-90, 0, 0));
					...
					remoteIcon.SetBool("on", false);
					grapple.showicon = false;
				}
			}
			omdown = mdown;
		}
